...
exports.guardarempleados = (req, res)=>{
    ...
    conexion.query('INSERT INTO empleadps SET ?',{ci:ci, cipadre:cipadre, nom1:nom1, nom2:nom2, ape1:ape1, ape2:ape2, sexo:sexo, direccion:direccion, lugarnac:lugarnac, teldomicilio:teldomicilio, email:email,etnia:etnia,estcivil:estcivil, colorpelo:colorpelo, estatura:estatura, peso:peso, idniveledu:idniveledu, cargo:cargo, contrato:contrato, departamento:departamento, ubicacion:ubicacion, estadofoto:estadofoto, clave:clave}, (error, results)=>{
        if(error){
            console.log(error);
        }else{
            console.log(results);   
